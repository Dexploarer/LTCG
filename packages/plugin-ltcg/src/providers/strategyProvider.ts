...
export const strategyProvider: Provider = {
  ...
  async get(runtime: IAgentRuntime, message: Memory, state: State): Promise<ProviderResult> {
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : 'Unknown error analyzing strategy';
...
interface StrategyAnalysis {
  ...
  priority: 'FINISH_GAME' | 'ESTABLISH_BOARD' | 'CLEAR_OPPONENT' | 'PROTECT_LIFE_POINTS' | 'SURVIVE';
