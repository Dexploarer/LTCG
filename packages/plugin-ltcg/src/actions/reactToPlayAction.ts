...
function classifyOpponentPlay(
  ...
  if (actionLower.includes('summon') && (actionLower.includes('dragon') || actionLower.includes('3000'))) {
    return {
      type: 'STRONG_PLAY',
      threatLevel: 'HIGH',
  ...
  if (actionLower.includes('negated') || actionLower.includes('failed') || actionLower.includes('blocked')) {
    return {
      type: 'FAILED_PLAY',
      threatLevel: 'LOW',
