...
import type { GameStateResponse, CardInHand, SpellTrapCard, GameEvent } from '../types/api';
...
export const chainResponseAction: Action = {
  ...
  validate: async (runtime: IAgentRuntime, message: Memory, state: State): Promise<boolean> => {
    try {
      ...
      const quickPlaySpells = hand?.filter((card) => {
  ...
  handler: async (
    ...
    try {
      ...
      // Format card options
      const handOptions = quickPlaySpells.length > 0
        ? quickPlaySpells
            .map((card, idx) => `Hand ${idx + 1}. ${card.name} - ${card.description?.substring(0, 100) || 'No description'}`)
            .join('\n')
        : 'None';

      const trapOptions = setTraps.length > 0
        ? setTraps
