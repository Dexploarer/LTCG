  ...
  handler: async (ctx, args) => {
    ...
    const chaptersWithProgress = allChapters.map((chapter) => {
      const progress = allProgress.find(
        (p) => p.actNumber === chapter.actNumber && p.chapterNumber === chapter.chapterNumber
      );

      // Get stages count (for now, all chapters have 10 stages)
      const stagesCompleted = 0; // TODO: Calculate from stage progress
...
export const getPlayerXPInfo = query({
  ...
  handler: async (ctx, args) => {
    ...
    const xpForNextLevel = xpRecord.currentLevel >= 100
      ? 0
      : (XP_PER_LEVEL[xpRecord.currentLevel] ?? 0);
